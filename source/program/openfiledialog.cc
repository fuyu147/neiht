...
static void on_portal_response(GDBusConnection *connection, const gchar *sender_name, const gchar *object_path,
                               ...
                               gpointer user_data)
{
        ...
        if (response == 0)
        { // Success
                GVariant *uri_list_variant = g_variant_lookup_value(results, "uris", G_VARIANT_TYPE("as"));
                if (uri_list_variant)
                {
                        const gchar **uri_list = g_variant_get_strv(uri_list_variant, nullptr);
                        if (uri_list)
                        {
...
std::vector<std::string> proj::program::getFileFromDialog()
{
        ...
        g_variant_builder_add(&filters, "(s@a(us))", "Text files", g_variant_builder_end(&mime_types));

        g_variant_builder_add(&options_builder, "{sv}", "filters", g_variant_builder_end(&filters));

        g_variant_builder_add(&options_builder, "{sv}", "multiple", g_variant_new_boolean(TRUE));

        result_variant = g_dbus_proxy_call_sync(proxy, "OpenFile",
                                                g_variant_new("(ss@a{sv})",
                                                              "", // parent_window
                                                              "Select a file", g_variant_builder_end(&options_builder)),
